...
namespace celionatti\Voltage\Router;
        ...
        if (is_string($callback)) {
            ...
            if (count($callbackParts) === 2) {
                ...
                if (!method_exists($controllerName, $actionName)) {
                    throw new VoltException("[{$controllerName}] - [{$actionName}] Method Not Found");
                }

                // Create the controller instance
                $controllerClass = "\\Voltage\\app\\controllers\\$controllerName";
                $controller = new $controllerClass();
                $controller->action = $actionName;

                // Set the controller in your application (you'll need to modify this according to your application's structure)
                Voltage::$voltage->controller = $controller;
