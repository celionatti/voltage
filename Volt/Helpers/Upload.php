...
namespace celionatti\Voltage\Helpers;
...
class Upload
{
    ...
    public function uploadFile(string $fileInputName, bool $rename = true): array
    {
        ...
        return ['error' => 'Unknown error during file upload.'];
    }
    ...
    // Add this method to the Upload class
    public function isFileUploaded(string $fileInputName): bool
    {
        if (isset($_FILES[$fileInputName]) && !empty($_FILES[$fileInputName]['name']) && !empty($_FILES[$fileInputName]['tmp_name'])) {
            return true;
        }
