...
namespace celionatti\Voltage\Exceptions;
...
class VoltException extends Exception
{
    ...
    private function logErrorToFile($maxLogSizeBytes = 1048576)
    {
        ...
        $basePath = rootDir() . DIRECTORY_SEPARATOR . "logs" . DIRECTORY_SEPARATOR;
        if (!is_dir($basePath)) {
            // Create the controller directory
            if (!mkdir($basePath, 0755, true)) {
                consoleLogger("Error: Unable to create the Logs directory.", true, true, 'error');
            }
        }
    ...
    private function displayErrorOnScreen()
    {
        ...
        echo '<p style="text-align:center;">copyright Voltage.</p>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</body>';
